...
def valid_int(n):
    if not n.isdigit():
        raise ArgumentTypeError('mod bir sayi olmalidir')

    n = int(n)

    if n > 3:
        raise ArgumentTypeError('modun max. degeri 3 olmalidir')

    if n < 0:
        raise ArgumentTypeError('modun min. degeri 0 olmalidir')
...
def args():
    args = ArgumentParser()
    args.add_argument('username', help='email veya kullanici adi')
    args.add_argument('passlist', help='sifre listesi')
    args.add_argument('-nc', '--no-color', dest='color',
                      action='store_true', help='renkler devre disi')
